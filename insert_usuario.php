...
if ($fragErro) {
    ...
    try {
        ...
        if ($insert) {
            header('Location: novo_usuario.php?msg='.urlencode('<p class="okMsg">O usu&aacute;rio foi adicionado!</p>'));
        } else {
            header('Location: novo_usuario.php?msg='.urlencode('<p class="errorMsg">Erro ao adicionar o usu&aacute;rio!</p>'));
        }
    } catch ( Exception $e ){
        header('Location: novo_usuario.php?msg='.urlencode('<p class="errorMsg">Erro ao adicionar o usu&aacute;rio!</p>'));
    }
} else {
    header('Location: novo_usuario.php?msg='.urlencode('<p class="errorMsg">'.$mensagem.'</p>'));
}
mysql_close($conexao);
